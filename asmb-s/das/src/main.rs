...
/*
	 This disassembler will take hex input cooresponding to one of the 
...
fn main() {
	...
	if args.len() == 1 || args[1].trim()  == "-h" {
		println!("Usage: das <ISA> <hex string>");
		return;
	}
	let isa = &args[1].trim();
	let data = &args[2].trim();
	let assem_path = inst_flag(data);
	let m_isa : &str = &(isa.trim()).to_lowercase().to_string();
	let mut dump:Vec<&str> = match m_isa {
		"arm32" =>	vec!["/usr/bin/arm-none-eabi-objdump","-z","--no-show-raw-insn","-EL","-D"],
...
/*
	 Hex data read from instruction input and converted to binary format.
 */
fn inst_flag(inst: &str) -> &str {
	...
	let mut output_buf = vec![0u8;inst.len() /2];
	let bin = match hex2bin((inst.trim()).as_bytes(), &mut output_buf) {
		Ok(bin) => bin,
			Err(err) => {
				println!("Unable to convert string hex to binary. Error: {:?}",err);
				return "file_error"
			}
	};
	match inst_file.write(bin) {
		Ok(num_bytes) => num_bytes,
			Err(err) => {
				println!("Unable to write to file \"inst.o\". Error: {}",err);
				return "file_error"
			}
	};
